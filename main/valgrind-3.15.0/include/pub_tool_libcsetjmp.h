...
#define VG_MINIMAL_SETJMP(_env)   my_local_setjmp((_env))
#define VG_MINIMAL_LONGJMP(_env)  my_local_longjmp((_env),1)

static __attribute__((always_inline)) inline int my_local_setjmp(void **env) {
    long double *fenv = (long double *)(env + 14);
    __asm__ __volatile__ (
        ...
        "stp d12, d13, [%1, #32]\n\t"
        "stp d14, d15, [%1, #48]\n\t"
        : : "r" (env), "r" (fenv) : "x1", "memory"
    );
    return 0;
}

static __attribute__((always_inline)) inline void my_local_longjmp(void **env, int val) {
    long double *fenv = (long double *)(env + 14);
    __asm__ __volatile__ (
        "ldp x19, x20, [%0, #0]\n\t"
        "ldp x21, x22, [%0, #16]\n\t"
        ...
        "mov x0, %1\n\t"
        "br x30\n\t"
        : : "r" (env), "r" (fenv), "r" (val) : "x0", "x2", "memory"
    );
}
